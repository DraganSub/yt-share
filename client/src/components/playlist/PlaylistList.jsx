...
export default function PlaylistList(data) {
  ...
  const playVideo = async (video) => {
    await updateData(`rooms/${localStorage.getItem("room_key")}`, { specificVideo: video.videoId, currentTime: 0, isPlaying: true });
  }
  ...
  const removeVideoFromPlaylist = async (video) => {
    ...
    if (entryId) {
      await removeData(`rooms/${localStorage.getItem("room_key")}/playList/${entryId}`);
    }

    if (video.videoId === databaseData.specificVideo) {
      if (Object.values(databaseData.playList).length > currentVideoIndex + 1) {
        await updateData(`rooms/${localStorage.getItem("room_key")}`, { specificVideo: Object.values(databaseData.playList)[currentVideoIndex + 1].videoId, currentTime: 0, isPlaying: true });
      } else {
